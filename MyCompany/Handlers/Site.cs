...
namespace MyCompany.Handlers
{
    ...
    public class SiteBase : MyCompany.Web.PageBase
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (_isTouchUI)
            {
                if (_pageFooterContent != null)
                	_pageFooterContent.Text = "<footer style=\"display:none\"><small>&copy; 2015 MyCompany. ^Copyright^All rights " +
            ...
            if (!(ApplicationServices.UserIsAuthorizedToAccessResource(HttpContext.Current.Request.Path, _bodyAttributes["data-authorize-roles"])))
            {
                ...
                if (WorkflowRegister.IsEnabled)
                {
                    if (!(WorkflowRegister.Allows(requestPath)))
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            sm.ResolveScriptReference += new EventHandler<ScriptReferenceEventArgs>(this.sm_ResolveScriptReference);
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            if (_isTouchUI)
            {
                ...
                content = Regex.Replace(content, "\\s*type=\"text/javascript\"\\s*", String.Empty);
            }
