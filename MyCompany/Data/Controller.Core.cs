...
#pragma warning restore 0618
            ...
            if (o != null)
            	if (o.GetType() == typeof(List<object[]>))
                	...
                	if ((o is Array) && (o.GetType().GetElementType() == typeof(object)))
                    {
                    	...
                    	if (o is DateTime)
                        	return ((DateTime)(o)).ToString("yyyy-MM-ddTHH\\:mm\\:ss.fffZ");
            return o;
        }
        ...
        protected virtual bool ValidateViewAccess(string controller, string view, string access)
        {
            ...
            if ((!(executionFilePath.StartsWith("~/appservices/", StringComparison.OrdinalIgnoreCase)) && !(executionFilePath.Equals("~/blob.ashx", StringComparison.OrdinalIgnoreCase))) && !(executionFilePath.Equals("~/charthost.aspx", StringComparison.OrdinalIgnoreCase)))
            {
