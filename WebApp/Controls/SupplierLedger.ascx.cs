...
public partial class Controls_SupplierLedger : System.Web.UI.UserControl
{
    ...
    public void getACCsubCode()
    {
        ...
        con = new SqlConnection(conr);
        SqlDataAdapter adpt = new SqlDataAdapter("SELECT   [AccSubCode], AccSubName FROM [dbo].[AccSubNames] where AccSubTypeID=4 and AccFundID="+accfund+" Order by AccSubName ", con);
        DataTable dt = new DataTable();
        adpt.Fill(dt);
    ...
    private void showReport()
    {
        ...
        {
            lblerror.Text = "Please Select Supplier Ledger ";
            return;
        }
        ...
        try
        {
           ...
           // ReportViewer1.LocalReport.ReportPath = new DirectoryInfo(HttpContext.Current.Server.MapPath("~/")).Parent.FullName + "\\MyCompany\\Reports\\USPSUBSIDIARY_Ledger.rdlc";
            ...
            fileName = "Supplier_Ledger" + DateTime.Now.ToFileTime() + ".pdf";
    ...
    protected void return_BalancheRptForm_Click(object sender, EventArgs e)
    {
        ...
        rptHeader.Visible = false;
        clear();
        Response.Redirect("~/Pages/SupplierLedger.aspx");
    }
}
