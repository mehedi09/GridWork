...
public partial class Controls_CustomerLedger : System.Web.UI.UserControl
{
    ...
    public void getACCsubCode()
    {
        ...
        con = new SqlConnection(conr);
        SqlDataAdapter adpt = new SqlDataAdapter("SELECT   [AccSubCode], AccSubName FROM [dbo].[AccSubNames] where AccSubTypeID=3 and AccFundID="+accfund+" Order by AccSubName ", con);
        DataTable dt = new DataTable();
        adpt.Fill(dt);
        ...
        ddlAccSubCode.Items.Insert(0, new ListItem("--Select Ledger Account--", ""));
        if (dt.Rows.Count > 0)
        {
    ...
    public void getACCCode()
    {
        ...
        ddlACCCode.DataBind();
        ddlACCCode.Items.Insert(0, new ListItem("--Select Customer Ledger --", ""));
    ...
    private void showReport()
    {
        ...
        try
        {
            ...
            //ad.GetProperty("Session_CompanyID").ToString();
            int accfund = 3;
            if (!string.IsNullOrEmpty(ad.CompanyID.ToString()))
            {
                accfund = int.Parse(ad.CompanyID.ToString());
            ...
            ReportViewer1.LocalReport.ReportPath = new DirectoryInfo(Server.MapPath("/Controls/")) + "USPSUBSIDIARY_Ledger.rdlc";
            ...
            fileName = "Customer_Ledger" + DateTime.Now.ToFileTime() + ".pdf";
    ...
    protected void return_BalancheRptForm_Click(object sender, EventArgs e)
    {
        ...
        rptHeader.Visible = false;
        clear();
        Response.Redirect("~/Pages/CustomerLedger.aspx");
    }
